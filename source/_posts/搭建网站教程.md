...
2. 在Github上创建Repository，Repository  name 中填入`username.github.io`，点击Create repository
   ...
   >
   > 但在[2020-10-01后，github新建的默认的库位main而不是master](https://www.jiqizhixin.com/articles/2020-09-21-10)，会导致在`hexo g -d`后，网页一直不更新，此时就需要在`GitHub Pages`部分的`Source`中把分支切换为`master`。
   >
...
3. 新建一个文件夹XXXX，在此文件夹下右键 `Git bash here`，输入：

   ```
   $ git clone 复制的ssh
   ```

4. 将该文件夹下除了.git（默认是隐藏文件）的文件夹全部删除
...
1. 在根目录下右键并GIt Bash here，然后输入`hexo new page tags`，在`~/source/tags/`出现了`index.md`，打开在Front-matter部分增加`type: "categories"`	
...
4. 在`主题配置文件`中，将`enable`修改为`true`，就可以在页面添加统计模块。
...
- `posts`文章字体
- `codes`代码字体
  - 每一项都有一个额外的 `external` 属性，此属性用来控制是否使用外链字体库。 
  - `family`设置字体
  - `size`默认是1
...
在`~\themes\hexo-theme-next\source\css\_variables\base.styl`中的`Font size`模块里修改`font-size-base`，默认为1。

### 修改行高

在`~\themes\hexo-theme-next\source\css\_variables\base.styl`中，默认分别是2和1.6
